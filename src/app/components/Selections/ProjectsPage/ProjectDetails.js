...
const ProjectDetails = ({project}) => {
  ...
  return (
      <div className='flex flex-col items-center text-emerald-950'>
        ...
        <h1 className='text-base md:text-2xl bg-slate-300/90 border-2 border-slate-100/50 rounded drop-shadow-sm p-1 md:p-2 my-2'>Abstract:</h1>
        <p className='flex flex-col text-xs md:text-lg indent-6 w-[90vw] md:w-2/3 bg-slate-300/90 rounded justify-center border-2 border-slate-100 px-4 py-2 m-4'>{abstract}</p>
        <h3 className='project-heading text-slate-100/75 underline'>Tech Stack Used:</h3>
        <motion.span 
        className='flex w-4/5 md:w-3/5 justify-between mx-2 my-2 md:my-4 p-2 text-4xl md:text-6xl bg-slate-200/75 rounded drop-shadow-lg border-2 border-slate-100'>
          {projectStack}
        </motion.span>
      </div>
  )
