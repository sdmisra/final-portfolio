...
const ProjectDetails = ({project}) => {
  ...
  return (
      <div className='flex flex-col items-center text-emerald-950'>
        ...
        <p className='flex flex-col bg-slate-200/60 text-xs md:text-lg indent-6 w-[90vw] md:w-2/3 justify-center px-4 py-2 m-4'>{abstract}</p>
        <h3 className='underline text-slate-100/75'>Tech Stack Used:</h3>
        <motion.span 
        className='flex w-4/5 md:w-3/5 justify-between mx-2 my-2 md:my-4 p-2 text-4xl md:text-6xl rounded-lg bg-slate-100/50'>
          {projectStack}
        </motion.span>
      </div>
  )
